...
class Database:
    ...
    def __call__(cls, *args, **kwargs):
        if cls not in cls._instances:
            cls._instances[cls] = super(Database, cls).__call__(*args, **kwargs)
    ...
    def __init__(self):
        ...
        if os.path.getsize(self._path) == 0:
            for media_type in MediaType:
                ...
                self._fields.update({media_type.value: ['title', 'aliases', 'year', 'created_at', 'updated_at']})
        else:
            for media_type in MediaType:
    ...
    def add(self, table: str, title: str, year: str, alias: str = None, **kwargs):
        if item := self.get(table=table, title=title, alias=alias, year=year):
            ...
            self.cursor.execute(f"""
                ...
                VALUES ('{title}', '{alias + ', ' if alias else ''}', '{year}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
            """)
            self.conn.commit()
    ...
    def _update(self, table: str, item: dict, alias: str = None, **kwargs):
        if alias and alias not in item['aliases']:
            self._update_fields(table=table, item=item, name='aliases', value=f"{alias}, {item['aliases']}")
        ...
        for key, value in kwargs.items():
            if not item.get(key): # or str(item[key]) != str(value):
                ...
                log(f"Field '{key}' already set for '{item['title']} ({item['year']})' in {table}", level='DEBUG')

    def get(self, table: str, title: str, alias: str, year: str) -> Any:
        with self._lock:
            self.cursor.execute(f"SELECT * FROM {table} WHERE (title='{title}' OR aliases LIKE '%{title},%') AND year='{year}'")
            item = self.cursor.fetchone()
            if item:
                return {f"{field}": item[i]  for i, field in enumerate(self._fields[table])}
            if not alias or alias == '':
                return None
            self.cursor.execute(f"SELECT * FROM {table} WHERE (title='{alias}' OR aliases LIKE '%{alias},%') AND year='{year}'")
            item = self.cursor.fetchone()
            if item:
                return {f"{field}": item[i]  for i, field in enumerate(self._fields[table])}
            return None
